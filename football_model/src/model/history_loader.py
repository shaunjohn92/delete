...
class HistoryDataLoader:
    ...
    def _infer_role_from_grid(
        ...
        """
        Very lightweight grid-to-role mapping.

        We derive:
            role: CB/FB/DM/CM/W/ST/GK/UNK
            side: L/C/R/UNK
            band: deep/mid/high/UNK
            role_profile: more descriptive string
        """
        # Defaults
        role = "UNK"
        side = "UNK"
        ...
        # Quick pos-based hints
        if pos in {"GK"}:
            role = "GK"
        elif pos in {"CB", "RCB", "LCB"}:
            role = "CB"
        elif pos in {"FB", "RB", "RWB", "LB", "LWB"}:
            role = "FB"
        elif pos in {"DM", "CDM"}:
            role = "DM"
        elif pos in {"CM", "RCM", "LCM", "AM", "CAM"}:
            role = "CM"
        elif pos in {"RW", "LW", "RM", "LM"}:
            role = "W"
        elif pos in {"ST", "CF"}:
            role = "ST"
